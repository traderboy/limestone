...
exports.SphinxClient = function() {
    ...
    self.query = function(query, callback) {
        ...
        if( sdebug ) console.log('Limestone query:', JSON.stringify(query,null,4));
        ...
        request.push.int32(query.length);
        ...
        for(var i = 0; i < query.length; i++) {
            ...
            for (var prop in query_parameters) {
                if (query[i].hasOwnProperty(prop)) {
                    ...
                    extendedQuery[prop] = query_parameters[prop];
                }
            }

            this.addQuery(request,extendedQuery);
        }
        ...
        _enqueue(request_buf, function(err,responses){
            var output = (responses && responses.length == 1)? responses[0] : responses;

            callback(err, output);
        }, Sphinx.clientCommand.SEARCH);
    };

    self.addQuery = function(request,query){
            ...
            runCallbackIfDone : function() {
                if (this.done()) {
                    ...
                    if (!errmsg) {
                        ...
                        if( sdebug ) console.log('limestone: answer.length ', JSON.stringify(answer,null,4))
                    }
    ...
    var parseExcerptResponse = function (data) {
        ...
        var output = {'docs':[]};
        var response = data.toReader();
        if( sdebug ) console.log('limestone: parseExcerptResponse response ', JSON.stringify(response,null,4));
        while(!response.empty()) {
